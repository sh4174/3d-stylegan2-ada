...
from metrics_3d import metric_defaults
...
def setup_training_options(
    ...
    # Initialize dicts.
    args = dnnlib.EasyDict()
    args.G_args = dnnlib.EasyDict(func_name='training.networks_3d.G_main')
    args.D_args = dnnlib.EasyDict(func_name='training.networks_3d.D_main')
    args.G_opt_args = dnnlib.EasyDict(beta1=0.0, beta2=0.99)
    args.D_opt_args = dnnlib.EasyDict(beta1=0.0, beta2=0.99)
    args.loss_args = dnnlib.EasyDict(func_name='training.loss_3d.stylegan2')
    args.augment_args = dnnlib.EasyDict(class_name='training.augment_3d.AdaptiveAugment')
    ...
    with tf.Graph().as_default(), tflib.create_session().as_default(): # pylint: disable=not-context-manager
        ...
        dataset_obj = dataset_3d.load_dataset(**args.train_dataset_args) # try to load the data and see what comes out
    ...
    cfg_specs = {
        ...
        'stylegan2-3d':     dict(ref_gpus=8,  kimg=6000,  mb=16, mbstd=2,  fmaps=1,   lrate=0.002,  gamma=10,   ema=10,  ramp=None, map=8), # uses mixed-precision, unlike original StyleGAN2
        'stylegan2-3d-anisobase':     dict(ref_gpus=4,  kimg=6000,  mb=8, mbstd=2,  fmaps=1,   lrate=0.002,  gamma=0,   ema=10,  ramp=None, map=8), # uses mixed-precision, unlike         'paper256':      dict(ref_gpus=8,  kimg=25000,  mb=64, mbstd=8,  fmaps=0.5, lrate=0.0025, gamma=1,    ema=20,  ramp=None, map=8),
    ...
    if cfg == 'stylegan2-3d':
        args.G_args.architecture = 'orig' # disable residual skip connections
        args.D_args.architecture = 'resnet' # disable residual skip connections
        args.G_args.fmap_max = args.D_args.fmap_max = 128
        args.G_args.fmap_min = args.D_args.fmap_min = 16
        args.G_args.fmap_decay = args.D_args.fmap_decay = 1

    if cfg == 'stylegan2-3d-anisobase':
        args.G_args.architecture = 'orig' # disable residual skip connections
        args.D_args.architecture = 'resnet' # disable residual skip connections
        args.G_args.fmap_base = args.D_args.fmap_base = int(spec.fmaps * ( 32 * 128 ) )

        args.G_args.fmap_max = args.D_args.fmap_max = 128 # Memory restriction
        args.G_args.fmap_min = args.D_args.fmap_min = 12 # Memory restriction
        args.G_args.fmap_decay = args.D_args.fmap_decay = 1.6 # Memory restriction
        args.G_args.base_dim = args.D_args.base_dim = [ 5, 6, 7 ]  # anisotropic base
        args.minibatch_size = spec.mb
        args.minibatch_gpu = spec.mb // spec.ref_gpus

        # args.loss_args.pl_weight = 2.0
        # args.loss_args.r1_gamma = 0
    ...
    if aug != 'noaug':
        args.augment_args.apply_func = 'training.augment_3d.augment_pipeline'
    ...
    if cmethod == 'nocmethod':
        ...
        args.loss_args.bcr_augment = dnnlib.EasyDict(func_name='training.augment_3d.augment_pipeline', xint=1, xint_max=1/32)
...
def run_training(outdir, seed, dry_run, **hyperparam_options):
    ...
    with dnnlib.util.Logger(os.path.join(training_options.run_dir, 'log.txt')):
        training_loop_3d.training_loop(**training_options)
...
def main():
    ...
    group.add_argument('--cfg',   help='Base config (default: auto)', choices=['auto', 'stylegan2', 'stylegan2-3d', 'stylegan2-3d-anisobase', 'paper256', 'paper512', 'paper1024', 'cifar', 'cifarbaseline'])
