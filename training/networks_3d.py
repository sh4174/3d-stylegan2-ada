...
def get_weight(shape, gain=1, equalized_lr=True, lrmul=1, weight_var='weight', trainable=True, use_spectral_norm=False):
    fan_in = np.prod(shape[:-1]) # [kernel, kernel, kernel, fmaps_in, fmaps_out] for conv3d, [in, out] for fully-connected.
...
#----------------------------------------------------------------------------
# 3d convolution op with optional upsampling, downsampling, and padding.

def conv3d(x, w, up=False, down=False, resample_kernel=None, padding=0):
    ...
    w = tf.cast(w, x.dtype)
    if up:
        x = upsample_conv_3d(x, w, data_format='NCDHW', k=resample_kernel, padding=padding)
    elif down:
        x = conv_downsample_3d(x, w, data_format='NCDHW', k=resample_kernel, padding=padding)
    else:
        padding_mode = {0: 'SAME', -(kernel // 2): 'VALID'}[padding]
        x = tf.nn.conv3d(x, w, data_format='NCDHW', strides=[1,1,1,1,1], padding=padding_mode)
    return x

#----------------------------------------------------------------------------
# 3d convolution layer.

def conv3d_layer(x, fmaps, kernel, up=False, down=False, resample_kernel=None, lrmul=1, trainable=True, use_spectral_norm=False):
    w = get_weight([kernel, kernel, kernel, x.shape[1].value, fmaps], lrmul=lrmul, trainable=trainable, use_spectral_norm=use_spectral_norm)
    return conv3d(x, tf.cast(w, x.dtype), up=up, down=down, resample_kernel=resample_kernel)

#----------------------------------------------------------------------------
# Modulated 3d convolution layer from the paper
...
def modulated_conv3d_layer(x, y, fmaps, kernel, up=False, down=False, demodulate=True, resample_kernel=None, gain=1, use_wscale=True, lrmul=1, fused_modconv=True, weight_var='weight', mod_weight_var='mod_weight', mod_bias_var='mod_bias'):
    ...
    # print( "=====================================================" )
    # print( "modulated_conv3d_layer, kernel" )
    # print( kernel )
    # print( "=====================================================")
    # print( "x.shape" )
    # print( x.shape )
    ...
    # Get weight.
    w = get_weight([kernel, kernel, kernel, x.shape[1].value, fmaps], gain=gain, lrmul=lrmul, weight_var=weight_var)
    ...
    #     print( "conv3d w" )
    #     print( w.shape )
    #     print( "conv3d x.dtype" )
    #     print( x.dtype )

    #     x = tf.nn.conv3d(x, tf.cast(w, x.dtype), data_format='NCDHW', strides=[1,1,1,1,1], padding='SAME')
    x = conv3d(x, tf.cast(w, x.dtype), up=up, down=down, resample_kernel=resample_kernel)

    # print( "=========================================================" )
    # print( "reshape/scale output" )
    # print( "x" )
    # print( x.shape )
    # print( "fmaps" )
    # print( fmaps )
...
def G_synthesis(
    ...
    # Base dimension for anisotropic inputdata
    base_dim = [ 4, 4, 4 ],
    ...
    resample_kernel     = [1,3,3,3,1],    # Low-pass filter to apply when resampling activations, None = box filter.
    fused_modconv       = False,        # Implement modulated_conv3d_layer() using grouped convolution?
    ...
    **_kwargs,                          # Ignore unrecognized keyword args.
    ...
    if use_noise:
        for layer_idx in range(num_layers - 1):
            res = (layer_idx + 5) // 2
            shape = [1, 1, int( base_dim[ 0 ] * 2**(res-2) ), int( base_dim[1 ] * 2**(res-2)), int( base_dim[ 2 ] * 2**(res-2) ) ]
    ...
    def upsample(y):
        with tf.variable_scope('Upsample'):
            return upsample_3d(y, k=resample_kernel)

    # ToRGB block.
    def torgb(x, y, res): # res = 2..resolution_log2
        with tf.variable_scope('ToRGB'):
            t = modulated_conv3d_layer(x, dlatents_in[:, res*2-3], fmaps=num_channels, kernel=1, demodulate=False, fused_modconv=fused_modconv)
    ...
    # Layers for base_dim resolution.
    y = None
    with tf.variable_scope(f'{base_dim[0]}x{base_dim[1]}x{base_dim[2]}'):
        with tf.variable_scope('Const'):
            fmaps = fmap_const if fmap_const is not None else nf(1)
            x = tf.get_variable('const', shape=[1, fmaps, base_dim[0], base_dim[1], base_dim[2]], initializer=tf.initializers.random_normal())
    ...
    for res in range(3, resolution_log2 + 1):
        with tf.variable_scope(f'{base_dim[0] * 2**(res-2)}x{ base_dim[1] * 2**( res - 2 ) }x{ base_dim[2 ] * 2**(res-2 )}'):
            x = block(x, res)
            if architecture == 'skip':
                y = upsample(y)
...
def D_main(
    ...
    **_kwargs,                          # Ignore unrecognized keyword args.
    ...
    # Inputs.
    images_in.set_shape([None, num_channels, int( resolution / 4 * base_dim[ 0 ] ), int( resolution / 4 * base_dim[ 1 ] ), int( resolution / 4 * base_dim[ 2 ] ) ] )
    labels_in.set_shape([None, label_size])
    ...
    def adrop(x):
        if adaptive_dropout != 0:
            s = [tf.shape(x)[0], x.shape[1], x.shape[ 2 ] ] + [1] * (x.shape.rank - 2)
    ...
    def fromrgb(x, y, res): # res = 2..resolution_log2
        with tf.variable_scope('FromRGB'):
            ...
            t = adrop(conv3d_layer(t, fmaps=nf(res-1), kernel=1, trainable=trainable))
    ...
    def block(x, res): # res = 2..resolution_log2
        ...
        with tf.variable_scope('Conv0'):
            trainable = is_next_layer_trainable()
            x = apply_bias_act(adrop(conv3d_layer(x, fmaps=nf(res-1), kernel=3, trainable=trainable, use_spectral_norm=use_spectral_norm)), act=act, clamp=conv_clamp, trainable=trainable)
        with tf.variable_scope('Conv1_down'):
            trainable = is_next_layer_trainable()
            x = apply_bias_act(adrop(conv3d_layer(x, fmaps=nf(res-2), kernel=3, down=True, resample_kernel=resample_kernel, trainable=trainable, use_spectral_norm=use_spectral_norm)), act=act, clamp=conv_clamp, trainable=trainable)
        if architecture == 'resnet':
            with tf.variable_scope('Skip'):
                trainable = is_next_layer_trainable()
                t = adrop(conv3d_layer(t, fmaps=nf(res-2), kernel=1, down=True, resample_kernel=resample_kernel, trainable=trainable))
                x = (x + t) * (1 / np.sqrt(2))
        return x
    ...
    # Layers for >=8x8x8 resolutions.
    x = None
    y = images_in
    for res in range(resolution_log2, 2, -1):
        with tf.variable_scope(f'{int( base_dim[ 0] * 2**( res- 2 ) )}x{int( base_dim[1 ] * 2**(res-2) )}x{int( base_dim[2] * 2**(res-2) ) }'):
            if architecture == 'skip' or res == resolution_log2:
                x = fromrgb(x, y, res)
            x = block(x, res)
            if architecture == 'skip':
    ...
    with tf.variable_scope(f'{base_dim[0]}x{base_dim[1]}x{base_dim[2]}'):
        ...
        with tf.variable_scope('Conv'):
            trainable = is_next_layer_trainable()
            x = apply_bias_act(adrop(conv3d_layer(x, fmaps=nf(1), kernel=3, trainable=trainable, use_spectral_norm=use_spectral_norm)), act=act, clamp=conv_clamp, trainable=trainable)
