...
def stylegan2(G, D, aug, fake_labels, real_images, real_labels, r1_gamma=10, pl_minibatch_shrink=2, pl_decay=0.01, pl_weight=2.0, **_kwargs):
    ...
    if r1_gamma != 0:
        with tf.name_scope('Loss_R1'):
            print( "===========================================================" )
            print( "R1 Regularizer" )
            print( "r1_grads = tf.gradients(tf.reduce_sum(D_real.scores), [real_images])[0]" )
            r1_grads = tf.gradients(tf.reduce_sum(D_real.scores), [real_images])[0]

            print( "r1_penalty = tf.reduce_sum(tf.square(r1_grads), axis=[1,2,3,4])" )
            r1_penalty = tf.reduce_sum(tf.square(r1_grads), axis=[1,2,3,4])

            print( "r1_penalty = report_stat(aug, 'Loss/r1_penalty', r1_penalty)" )
            r1_penalty = report_stat(aug, 'Loss/r1_penalty', r1_penalty)

            print( "D_reg += r1_penalty * (r1_gamma * 0.5)" )
