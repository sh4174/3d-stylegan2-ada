...
def translate_3d_inv(tx, ty, tz):
    return translate_3d(-tx, -ty, -tz)

def scale_3d_inv(sx, sy, sz):
    return scale_3d(1/sx, 1/sy, 1/sz)

def rotate_3d_inv(v, theta):
    return rotate_3d(-v, -theta)
...
def augment_pipeline(
    images,                         # Input images: NCHWD, float32, dynamic range [-1,+1].
    labels,                         # Input labels.
    ...
    batch, channels, height, width, depth = images.shape.as_list()
    if batch is None:
    ...
    # Initialize inverse homogeneous 3D transform: G_inv @ pixel_out ==> pixel_in
    ...
    #     G_inv @= rotate_3d_inv(0, -np.pi / 2 * i)

    # # Apply integer translation with probability (xint * strength).
    # if xint > 0:
    #     t = tf.random_uniform([batch, 3], -xint_max, xint_max)
    #     t = gate_augment_params(xint * strength, t, 0)
    #     if debug_percentile is not None:
    #         t = (tf.broadcast_to(debug_percentile, [batch, 3]) * 2 - 1) * xint_max
    #     G_inv @= translate_3d_inv(tf.rint(t[:,0] * width), tf.rint(t[:,1] * height), tf.rint(t[:,2] * depth))
    ...
    #     v = tf.random_uniform([batch], -np.pi * rotate_max, np.pi * rotate_max)
    #     v = gate_augment_params(p_rot, v, 0)
    #     if debug_percentile is not None:
    #         v = (tf.broadcast_to(debug_percentile, [batch]) * 2 - 1) * np.pi * rotate_max

    #     G_inv @= rotate_3d_inv( -v, -theta) # Before anisotropic scaling.
    ...
    #     G_inv @= rotate_3d_inv(-v, -theta) # After anisotropic scaling.

    # # Apply fractional translation with probability (xfrac * strength).
    # if xfrac > 0:
    #     t = tf.random_normal([batch, 3], 0, xfrac_std)
    #     t = gate_augment_params(xfrac * strength, t, 0)
    #     if debug_percentile is not None:
    #         t = tflib.erfinv(tf.broadcast_to(debug_percentile, [batch, 3]) * 2 - 1) * xfrac_std
    #     G_inv @= translate_2d_inv(t[:,0] * width, t[:,1] * height, t[:,2] * depth)
    ...
    #     Hz = np.reshape(Hz, [-1, 1, 1, 1]).repeat(channels, axis=1) # [tap, channel, 1]
    #     Hz_pad = Hz.shape[0] // 4
    ...
    #     cp = np.transpose([[-cx, -cy, -cz, 1], [cx, -cy, -cz, 1], [cx, cy, -cz, 1], [-cx, cy, -cz, 1]]) # [xyz, idx]
    #     cp = G_inv @ cp[np.newaxis] # [batch, xyz, idx]
    #     cp = cp[:, :2, :] # [batch, xyz, idx]
    #     m_lo = tf.ceil(tf.reduce_max(-cp, axis=[0,2]) - [cx, cy, cz] + Hz_pad * 2)
    #     m_hi = tf.ceil(tf.reduce_max( cp, axis=[0,2]) - [cx, cy, cz] + Hz_pad * 2)
    #     m_lo = tf.clip_by_value(m_lo, [0, 0], [width-1, height-1, depth-1])
    #     m_hi = tf.clip_by_value(m_hi, [0, 0], [width-1, height-1, depth-1])

    #     # Pad image and adjust origin.
    #     images = tf.transpose(images, [0, 2, 3, 4, 1]) # NCHWD => NHWDC
    ...
    #     images = tf.transpose(images, [0, 4, 1, 2, 3 ]) # NHWC => NCHW
    ...
    # Execute if the transform is not identity.
    if C is not I_4:
        images = tf.reshape(images, [batch, channels, height * width * depth])
        if channels == 3:
    ...
    if noise > 0:
        ...
        images += tf.random_normal([batch, channels, height, width, depth]) * sigma
