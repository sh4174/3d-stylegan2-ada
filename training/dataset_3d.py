...
class TFRecordDataset:
    def __init__(self,
        ...
        self.base_dim           = None      # base_dim for anisotropic input data ( 192 x 128 x 192 or 160 x 192 x 224 )
        ...
        if np.max( max_shape ) == 224:
            self.base_dim = [ 5, 6, 7 ]
        elif np.max( max_shape ) == 192:
            self.base_dim = [ 6, 4, 6 ]
        else:
            self.base_dim = [ 4, 4, 4 ]


        self.resolution = int( np.max( max_shape ) / np.max( self.base_dim ) * 4 ) 
        # self.resolution = resolution if resolution is not None else int( np.max( max_shape ) / np.max( self.base_dim ) * 4 )
        self.resolution_log2 = int(np.log2(self.resolution))
        self.shape = [max_shape[0], int( self.resolution / 4 * self.base_dim[ 0 ] ), int( self.resolution / 4 * self.base_dim[ 1 ] ), int( self.resolution / 4 * self.base_dim[ 2 ] ) ]
        tfr_lods = [self.resolution_log2 - int(np.log2( shape[ 1 ] / self.base_dim[ 0 ] * 4 ) ) for shape in tfr_shapes]
