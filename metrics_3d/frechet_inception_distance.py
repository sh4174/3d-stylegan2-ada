...
class FID(metric_base.MetricBase):
    ...
    def _evaluate(self, Gs, G_kwargs, num_gpus, **_kwargs): # pylint: disable=arguments-differ
        ...
        fid_real_path = "/data/vision/polina/users/razvan/sungmin/stylegan2-ada/stylegan2-ada/sbatch/fid_real_img.pkl"
        ...
        if self.use_cached_real_stats and os.path.isfile(cache_file):
            ...
            for images, _labels, num in self._iterate_reals(minibatch_size):
                ...
                if len( images.shape ) == 5:
                    ...
                    sampled_axis = np.random.randint( 2, 5 )
                    sampled_slice = np.random.randint( slice_crop, images.shape[ sampled_axis ] - slice_crop )
                    images = np.take( images, sampled_slice, axis=sampled_axis )

                    # if sampled_axis == 2:
                    #     images = images[ :, :, sampled_slice, :, : ]
                    # elif sampled_axis == 3:
                    #     images = images[ :, :, :, sampled_slice, : ]
                    # elif sampled_axis == 4:
                    #     images = images[ :, :, :, :, sampled_slice ]
            ...
            with open(fid_real_path, 'wb') as f:
                pickle.dump((mu_real, sigma_real), f)
